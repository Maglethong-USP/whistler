...
AS $body$
	...
	WHERE POST.DATA >= TO_TIMESTAMP($1, 'YYYY-MM-DD') AND POST.DATA <= TO_TIMESTAMP($2, 'YYYY-MM-DD')
...
/** ==================== Relatorio de Influencia Post ====================

 */
CREATE OR REPLACE FUNCTION RELATORIO_PUBLICACOES_TOP20(DATAINIC VARCHAR, DATAFIM VARCHAR)
	RETURNS TABLE (	NAME_USUARIO VARCHAR(64), INFLUENCIA INTEGER, ID_POST INTEGER, 
					CONTEUDO VARCHAR, DATA TIMESTAMP WITH TIME ZONE, RANKPOS INTEGER, RANKNEG INTEGER, 
					COMENTARIOS INTEGER, COMPARTILHAMENTOS INTEGER)
...
AS $body$
	SELECT 
		...
		POST.COMPARTILHAMENTOS AS "COMPARTILHAMENTOS"
		FROM USUARIO JOIN POST ON (USUARIO.ID = POST.ESCRITOR)
		WHERE POST.DATA >= TO_TIMESTAMP($1, 'YYYY-MM-DD') AND POST.DATA <= TO_TIMESTAMP($2, 'YYYY-MM-DD')
